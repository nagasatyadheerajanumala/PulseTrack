...
public class StatusPageService {
    ...
    public StatusAnalyticsResponse getAnalytics(String publicKey, String fromStr, String toStr) {
        StatusPage page = statusPageRepo.findByPublicKey(publicKey)
        ...
        List<Monitor> monitors = page.getMonitors();

        int totalChecks = 0;
        int totalFailures = 0;

        for (Monitor monitor : monitors) {
            List<MonitorLog> logs = monitorLogRepo.findByMonitorAndCheckedAtBetween(monitor, from, to);
            totalChecks += logs.size();
            totalFailures += logs.stream().filter(log -> log.getStatusCode() != 200).count();
        }

        double uptimePercentage = totalChecks == 0 ? 100.0 :
                ((double)(totalChecks - totalFailures) / totalChecks) * 100;

        return StatusAnalyticsResponse.builder()
